    ...
    while(cin >> num)
    {
        ...
        if(position!=-1)
        {
            ...
            cout << right  << setw(25) << this->rnumber[j] << right << setw(22) << this->rfloor[j]  <<
                 setw(22) << rcategory[j] << setw(30) << "Available"<< '\n';
            found=1;
            cout << "\n";
        }
        else
...
void hostel::room_close()
{
    ...
    ofstream sclear ("database/room.txt", std::ios::out | std::ios::trunc);
    sclear.close();

    int i;
    ofstream sinsert;
    sinsert.open("database/room.txt", std::ios_base::app);
...
{
    ofstream sclear ("database/information.txt", std::ios::out | std::ios::trunc);
    sclear.close();
}
...
void hostel::student_insert()
{
    int i;
    ofstream sinsert;
    sinsert.open("database/information.txt", std::ios_base::app);
...
void hostel::get_studentinfo()
{
    ...
    int flag=0;
    ifstream info;
    info.open("database/information.txt");
    while(!info.eof())
    {
